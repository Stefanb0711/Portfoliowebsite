import React from 'react';

function Home() {
    return (

        <div>

            <section id="hero" className="hero section dark-background">

                <div className="container">
                    <div className="row gy-4">
                        <div className="col-lg-6 order-2 order-lg-1 d-flex flex-column justify-content-center">
                            <h1>Hallo, ich bin Stefan</h1>
                            <p>Ein leidenschaftlicher Softwareentwickler</p>
                            <div className="d-flex">
                                <a href="/portfolio" className="btn-get-started">Zu meinem Portfolio</a>
                            </div>
                        </div>
                        <div className="col-lg-6 order-1 order-lg-2 hero-img">
                            <img src="assets/img/profilbildVonMirAusgeschnittenRund.png" className="img-fluid animated"
                                 alt=""/>
                        </div>
                    </div>
                </div>

            </section>

            <section id="hero" className="hero section">
                <div style={{marginTop: '100px', marginBottom: '100px'}} className="container">
                    <div className="row gy-4">
                        <div className="col-lg-6 order-2 order-lg-1 d-flex flex-column justify-content-center">
                            <h2>Todolist mit ASP.NET,Angular, Mongodb, Graphqhl, Tailwindcss</h2>

                            <a style={{marginTop: '30px'}} href={'https://github.com/Stefanb0711/todolistGraphqlC-Angular.git'}>https://github.com/Stefanb0711/todolistGraphqlC-Angular.git</a>
                            <button style={{width: '200px', marginTop: '50px'}} className="btn btn-get-started">Mehr erfahren</button>
                        </div>
                        <div className="col-lg-6 order-1 order-lg-2 hero-img">
                            <img className={"img-fluid"} src={'assets/img/FrontendBackend/TodolistAsp.Net/start.png'}/>

                        </div>
                    </div>
                </div>

            </section>

        </div>
);
}

export default Home;
